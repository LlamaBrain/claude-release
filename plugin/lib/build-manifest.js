...
if (isMain2) {
  const cli2 = parseCliOptions(process.argv.slice(2));
  const manifest2 = computeBump(cli2);
  const apply = process.argv.includes("--apply");
  console.log(JSON.stringify(manifest2, null, 2));
  if (apply && manifest2.next_version && manifest2.bump_kind !== "none") {
    writeFileSync("VERSION", `${manifest2.next_version}
`, "utf8");
    process.stderr.write(`Wrote VERSION <- ${manifest2.next_version}
`);
  }
}
...
import { existsSync, mkdtempSync, mkdirSync, copyFileSync, writeFileSync as writeFileSync2, rmSync } from "node:fs";
...
function hasDotnet() {
  try {
    execFileSync2("dotnet", ["--version"], { stdio: "ignore" });
    return true;
  } catch {
    return false;
  }
...
// src/classify-api-diff.js
function splitParamTuple(inner) {
  if (!inner) return [];
  const parts = [];
  let depth = 0;
  let start = 0;
...
function splitFqnAtOpenParen(fqn) {
  const i = fqn.indexOf("(");
  if (i < 0) return null;
  if (!fqn.endsWith(")")) return null;
  return {
    prefix: fqn.slice(0, i + 1),
    // includes the '('
    inner: fqn.slice(i + 1, -1)
    // strips the trailing ')'
  };
}
function isStrictAdditiveExtension(removedParams, addedParams) {
...
function classifyRemovedAddedPairs(apiDiff2) {
  const removed = apiDiff2.removed ?? [];
  const added = apiDiff2.added ?? [];
  const removedByPrefix = /* @__PURE__ */ new Map();
  const addedByPrefix = /* @__PURE__ */ new Map();
  ...
  const pairedRemovedFqns = /* @__PURE__ */ new Set();
  const additivePairs = [];
  for (const [prefix, removedItems] of removedByPrefix) {
    ...
    const claimedAdded = /* @__PURE__ */ new Set();
...
function hasStagedWork() {
  try {
    execFileSync3("git", ["diff", "--cached", "--quiet"], { stdio: "ignore" });
    return false;
  } catch {
    return true;
  }
}
function parseCliOptions2(argv) {
  const out = {};
  for (let i = 0; i < argv.length; i++) {
    const a = argv[i];
    if (a === "--prerelease" || a === "--pre") {
...
var cli = parseCliOptions2(process.argv.slice(2));
var manifest = computeBump(cli);
var apiDiff = await tryApiDiff(manifest.previous_version, "WORKTREE");
if (apiDiff) manifest.api_diff = apiDiff;
if (manifest.bump_kind === "none" && apiDiff && (apiDiff.added.length > 0 || apiDiff.removed.length > 0 || apiDiff.changed.length > 0)) {
  ...
  if (realRemoved.length > 0 || apiDiff.changed.length > 0) {
    promoted = "major";
    const pairNote = additivePairs.length > 0 ? ` (excluding ${additivePairs.length} additive-param pair(s))` : "";
    ...
    promoted = "minor";
    const pairNote = additivePairs.length > 0 ? `, ${additivePairs.length} of which extend(s) an existing signature with default-valued param(s)` : "";
