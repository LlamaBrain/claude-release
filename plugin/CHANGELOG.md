...
- **Release gates — a pre-stage gate phase in `/release` that refuses an inconsistent ship and records reason-carrying overrides.** Two gates in this minimal cut: `smoke_results_pass` reads the ATH `ath.smoke.completed` trace under `.captain-sdlc/trace/` for HEAD and blocks if the latest smoke failed or none ran for the commit; `dependency_audit` runs `npm audit --json` and blocks on high/critical, resolving to `not_applicable` when there is no `package.json` (e.g. Unity projects). `evaluate-gates.js` composes per-gate verdicts into a release decision and exits 0 (proceed) / 1 (blocked) / 2 (usage). Overrides are per-gate — `--force-release --override gate:"reason"` — with empty reasons, non-block gates, and `--override` without `--force-release` all rejected. Config is read from `.captain-sdlc/release-gates.yaml` with defaults when absent. `release.md` gains Step 3.5 and a `Gate-override` commit footer. (349d57d)
- **Release-readiness gate — `scripts/verify-release-readiness.mjs`, wired to `npm run check:release` and `prepare:release`.** Implements the script `package.json` already pointed at but that never existed. Two guards, each closing a bug class this project shipped: version consistency across all surfaces — `plugin/VERSION` is canonical and must match `.claude-plugin/marketplace.json`, `plugin/.claude-plugin/plugin.json`, `package.json`, and `package-lock.json` — and self-contained runtime, where every bundled `plugin/lib/` entry point must import nothing beyond Node builtins so an un-inlined `semver` / `conventional-commits-parser` can never ship broken again. Exits non-zero listing every problem; both guards were negative-tested. (41d28c3)
- **`scripts/entry-points.mjs`** — single source of truth for the published entry-point list, shared by `scripts/bundle.mjs` and the readiness gate so the two can never disagree about what ships. (41d28c3)
- **`.captain-sdlc/release-gates.yaml`** so claude-release can gate its own `/release`: `smoke_results_pass` disabled (no ATH smokes in this repo) and `dependency_audit` set to soft (warn) until trusted. (1524d52)

### Fixed

- **`npm audit` now runs on Windows.** Windows resolves `npm` to `npm.cmd`, and recent Node refuses to spawn a `.cmd` through `execFileSync` without a shell, so `dependency_audit` silently errored on every Windows run. The audit is now routed through a shell on win32 with a fixed argv and no user input; verified live, the gate runs a real `npm audit` and returns pass. (1524d52)
- **Smoke-test source imports repaired.** `test-api-diff-pair-smoke`, `test-smell-smoke`, and `test-api-diff-smoke` imported their targets as if sources still sat alongside the tests; v0.3.1's move to `src/` had left all three failing at module resolution. They now resolve `../src/` (and `../plugin/lib` for the dotnet ApiDiff project), and the four smoke suites pass. (742b825)
- **Version-surface drift synced to `0.3.2`.** `plugin/.claude-plugin/plugin.json` had lagged at `0.2.0` and `package.json` / `package-lock.json` at `0.3.1`, while `plugin/VERSION`, the git tag, and `marketplace.json` were already `0.3.2` — the same multi-version-field hazard `[0.3.2]` called out. The new release-readiness gate now fails the release if these diverge again. (c83c494)
- **Corrected the install location in the README.** (5e26f8a)
